...
Automatic rough transcription of Danish using [xls-r-300m-danish-nst-cv9](https://huggingface.co/chcaa/xls-r-300m-danish-nst-cv9/discussions) (ASR, automatic speech recognition), saving it as a CLAN (.cha) file. The transcript will be very rough and have weird spelling, but can be a help if you like to have a rough transcript with sound-linked bullets.
...
Download this repository (`git clone https://github.com/sorenss/transcribe-danish.git`). The sound file you want to have transcribed must be placed in the repository folder with the filename `transcribeme.wav`. From a terminal in this folder, run the bash script: `bash transcribedanish.sh`, and wait as it will take some time. When finished, the transcript will be saved to a text file called *transcribeme.cha*.
